...
,	Client( InClient )
{
	...
	DisplaySize.w = InClient->DefaultDisplayMode.w;
	DisplaySize.h = InClient->DefaultDisplayMode.h;
...
void UNSDLViewport::OpenWindow( void* InParentWindow, UBOOL Temporary, INT NewX, INT NewY, INT OpenX, INT OpenY )
{
	...
	else
	{
		...
		if( hWnd && ( DoOpenGL != !!SDL_GetWindowFlags( hWnd ) ) )
		{
			CloseWindow();
		}
